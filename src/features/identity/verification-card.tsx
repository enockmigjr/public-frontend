...
export function VerificationCard({ api = publicApi, onVerified }: { readonly api?: PublicApi; readonly onVerified?: () => void }) {
  ...
  return <Card className="w-full max-w-md border-slate-200 shadow-xl shadow-slate-950/5">
    <CardHeader><div className="mb-3 grid size-11 place-items-center rounded-xl bg-blue-100 text-blue-800">{challengeId ? <LockKeyhole /> : <Mail />}</div><CardTitle>{challengeId ? "Entrez le code reçu" : "Accédez à vos demandes"}</CardTitle><CardDescription>{challengeId ? `Nous avons envoyé un code à ${email}.` : "Aucun compte ni mot de passe requis. Votre adresse sert à retrouver vos demandes en toute sécurité."}</CardDescription></CardHeader>
    <CardContent>
